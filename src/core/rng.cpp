...
PhiloxCounter make_philox_counter(std::uint64_t path_index, std::uint32_t draw_index) noexcept {
...
double standard_normal_variate(std::uint64_t seed, std::uint64_t path_index,
    ...
    const PhiloxCounter counter = make_philox_counter(path_index, draw_index);
